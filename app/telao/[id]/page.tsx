...
function TelaoContent() {
  const { partida, pergunta, estado, configuracao, sincronizado, carregando, erro, resetarPartida } = useGame();

  if (carregando) return <main className="flex min-h-screen items-center justify-center bg-slate-950 text-white"><p>Carregando partida...</p></main>;
  if (erro) return <main className="flex min-h-screen items-center justify-center bg-slate-950 text-white"><p className="text-red-300">{erro}</p></main>;
  ...
  if (!pergunta) return <main className="flex min-h-screen items-center justify-center bg-slate-950 text-white"><div className="text-center"><h1 className="text-3xl font-black">Aguardando pergunta</h1><p className="mt-2 text-slate-400">O apresentador ainda não selecionou uma pergunta.</p><p className="mt-4 text-xs text-slate-600">{sincronizado ? "Sincronizado" : "Aguardando controlador"}</p></div></main>;

  return <main className="flex min-h-screen flex-col bg-slate-950 text-white">
    <header className="flex items-center justify-between border-b border-slate-800 px-5 py-3 sm:px-8"><div><p className="text-xs font-bold uppercase tracking-[0.3em] text-yellow-400">Decatlo</p><h1 className="text-xl font-black sm:text-2xl">{partida.nome}</h1></div><Placar equipe1={partida.equipe1} equipe2={partida.equipe2} score1={estado.pontos.equipe1} score2={estado.pontos.equipe2}/></header>
    <section className="flex flex-1 flex-col gap-5 px-4 py-5 sm:px-6 lg:px-10">
      <div className="flex items-center justify-center gap-3 sm:gap-6 lg:gap-12"><Mascote nome={partida.equipe1} video="/videos/mascote-alfa1.mp4" ativo={estado.equipeDaVez === "A"}/><Cronometro timeLeft={estado.tempoRestante}/><Mascote nome={partida.equipe2} video="/videos/mascote-alfa2.mp4" ativo={estado.equipeDaVez === "B"}/></div>
...
export default function TelaoPage() {
  ...
  return <GameProvider partidaId={params.id} role="display"><TelaoContent /></GameProvider>;
}
