...
export default function LoginForm({
  ...
  const inputClass = (hasError: boolean) =>
    `w-full rounded-xl border px-4 py-3 text-base text-white placeholder:text-white/35 outline-none transition focus:bg-white/[0.04] focus:ring-4 ${hasError ? "border-red-500/50 bg-red-500/5 focus:border-red-500/70 focus:ring-red-500/10" : "border-white/10 bg-white/[0.03] focus:border-cyan-400/70 focus:ring-cyan-500/10"}`;

  return (
    <form className="space-y-5" onSubmit={onSubmit}>
      {mensagem && (
        <div
          className={`rounded-lg p-4 text-sm font-medium ${mensagem.tipo === "sucesso" ? "border border-green-500/20 bg-green-500/10 text-green-300" : "border border-red-500/20 bg-red-500/10 text-red-300"}`}
        >
          {mensagem.texto}
        </div>
      )}
      <div>
        ...
        <input
          ...
          className={inputClass(Boolean(erros.usuario))}
        />
        {erros.usuario && (
      ...
      <div>
        ...
        <input
          ...
          className={inputClass(Boolean(erros.senha))}
        />
        {erros.senha && (
          <div className="mt-2 space-y-1">
            {erros.senha.map((erro) => (
              <p key={erro} className="text-xs text-red-400">
                • {erro}
              </p>
            ))}
          </div>
