...
type PartidaBanco = {
  ...
  /** * O Prisma retorna Date. * A API JSON normalmente entrega string. */ cronometroFimEm:
    | string
    | Date
    | null;
  data: string | Date;
...
export function GameProvider({ children, partidaId, role }: GameProviderProps) {
  ...
  const pausarPartida = useCallback(
    () => executarAcao({ type: "PAUSAR_PARTIDA" }),
    [executarAcao],
  );
  ...
  const mostrarResposta = useCallback(
    () => executarAcao({ type: "MOSTRAR_RESPOSTA" }),
    [executarAcao],
  );
  const esconderResposta = useCallback(
    () => executarAcao({ type: "ESCONDER_RESPOSTA" }),
    [executarAcao],
  );
  ...
  /** * Fallback usado enquanto a partida * ainda está sendo carregada. * * IMPORTANTE: * cronometroFimEm também precisa * existir aqui para satisfazer * PartidaBanco. */ const partidaSegura: PartidaBanco =
    partida ?? {
      id: partidaId,
      ...
      /** * Campo obrigatório do tipo * PartidaBanco. */ cronometroFimEm: null,
      data: new Date(),
