...
function criarEstado(partida: PartidaBanco): EstadoJogo {
  return {
    ...
    cronometroFimEm: partida.cronometroFimEm ? new Date(partida.cronometroFimEm).getTime() : null,
...
function aplicarAcao(estado: EstadoJogo, action: GameAction): EstadoJogo {
  switch (action.type) {
    case "INICIAR_PARTIDA":
      return { ...estado, status: "em_andamento", cronometroFimEm: action.cronometroFimEm };
    case "PAUSAR_PARTIDA":
      return { ...estado, status: "pausada", cronometroFimEm: null };
    case "FINALIZAR_PARTIDA":
      return { ...estado, status: "finalizada", cronometroFimEm: null };
    case "INICIAR_CRONOMETRO":
      return { ...estado, status: "em_andamento", cronometroFimEm: action.cronometroFimEm };
    case "PAUSAR_CRONOMETRO":
      return { ...estado, status: "pausada", tempoRestante: action.tempoRestante, cronometroFimEm: null };
    ...
    case "REMOVER_PONTO":
      return { ...estado, pontos: action.pontos };
    case "MOSTRAR_RESPOSTA":
      return { ...estado, respostaVisivel: true };
    case "ESCONDER_RESPOSTA":
      return { ...estado, respostaVisivel: false, resultado: null };
...
export function GameProvider({ children, partidaId, role }: GameProviderProps) {
  ...
  useEffect(() => {
    ...
    async function carregar() {
      try {
        setCarregando(true);
        setErro(null);
        const response = await fetch(`/api/partidas/${partidaId}`, { cache: "no-store" });
        const resultado = await response.json();
        if (!response.ok || !resultado.success) {
          throw new Error(resultado.error || "Não foi possível carregar a partida.");
        }
        if (!ativo) return;
        ...
        if (ativo) setErro(error instanceof Error ? error.message : "Não foi possível carregar a partida.");
      } finally {
  ...
  const persistirEstado = useCallback(async (novoEstado: EstadoJogo) => {
    try {
      await fetch(`/api/partidas/${partidaId}/estado`, {
        method: "PATCH",
  ...
  const executarAcao = useCallback((action: GameAction) => {
  ...
  useEffect(() => {
    if (!estado || estado.status !== "em_andamento" || estado.cronometroFimEm === null) return;

    const atualizar = () => {
      setEstado((atual) => {
        if (!atual?.cronometroFimEm) return atual;
        const restante = Math.max(0, Math.ceil((atual.cronometroFimEm - Date.now()) / 1000));
        if (restante <= 0) {
          const novo = { ...atual, tempoRestante: 0, cronometroFimEm: null, status: "pausada" as const };
          if (role === "control") void persistirEstado(novo);
          return novo;
        }
        return { ...atual, tempoRestante: restante };
      });
    };
  ...
  const pausarPartida = useCallback(() => executarAcao({ type: "PAUSAR_PARTIDA" }), [executarAcao]);

  const finalizarPartida = useCallback(() => executarAcao({ type: "FINALIZAR_PARTIDA" }), [executarAcao]);

  const iniciarCronometro = useCallback(() => {
    const tempo = estadoSeguro.tempoRestante > 0 ? estadoSeguro.tempoRestante : (partida?.tempoResposta ?? 30);
    executarAcao({ type: "INICIAR_CRONOMETRO", cronometroFimEm: Date.now() + tempo * 1000 });
  ...
  const pausarCronometro = useCallback(() => {
    ...
    if (estadoSeguro.cronometroFimEm !== null) {
      tempo = Math.max(0, Math.ceil((estadoSeguro.cronometroFimEm - Date.now()) / 1000));
    }
    executarAcao({ type: "PAUSAR_CRONOMETRO", tempoRestante: tempo });
  }, [estadoSeguro, executarAcao]);

  const reiniciarCronometro = useCallback(() => {
    executarAcao({ type: "REINICIAR_CRONOMETRO", tempoRestante: partida?.tempoResposta ?? 30 });
  }, [partida?.tempoResposta, executarAcao]);

  const proximaPergunta = useCallback(() => {
    if (!partida) return;
    if (estadoSeguro.perguntaAtual >= partida.perguntas) {
      executarAcao({ type: "FINALIZAR_PARTIDA" });
      return;
    }
    executarAcao({ type: "PROXIMA_PERGUNTA", perguntaAtual: estadoSeguro.perguntaAtual + 1, tempoRestante: partida.tempoResposta });
  }, [partida, estadoSeguro.perguntaAtual, executarAcao]);

  const perguntaAnterior = useCallback(() => {
    if (estadoSeguro.perguntaAtual <= 1 || !partida) return;
    executarAcao({ type: "PERGUNTA_ANTERIOR", perguntaAtual: estadoSeguro.perguntaAtual - 1, tempoRestante: partida.tempoResposta });
  }, [estadoSeguro.perguntaAtual, partida, executarAcao]);

  const pularPergunta = useCallback(() => {
    if (partida?.permitirPular) proximaPergunta();
  }, [partida?.permitirPular, proximaPergunta]);

  const trocarEquipe = useCallback(() => {
    executarAcao({ type: "TROCAR_EQUIPE", equipeDaVez: estadoSeguro.equipeDaVez === "A" ? "B" : "A" });
  }, [estadoSeguro.equipeDaVez, executarAcao]);

  const definirEquipe = useCallback((equipe: EquipeDaVez) => executarAcao({ type: "DEFINIR_EQUIPE", equipeDaVez: equipe }), [executarAcao]);

  const responder = useCallback((resultado: Exclude<ResultadoPergunta, null>) => {
    const pontos = { ...estadoSeguro.pontos };
    if (resultado === "correta") {
      if (estadoSeguro.equipeDaVez === "A") pontos.equipe1 += 1;
      else pontos.equipe2 += 1;
    }
    executarAcao({ type: "RESPONDER", resultado, pontos });
  }, [estadoSeguro, executarAcao]);

  const adicionarPonto = useCallback((equipe: EquipeDaVez, quantidade = 1) => {
    const pontos = { ...estadoSeguro.pontos };
    if (equipe === "A") pontos.equipe1 += quantidade;
    else pontos.equipe2 += quantidade;
    executarAcao({ type: "ADICIONAR_PONTO", equipe, quantidade, pontos });
  }, [estadoSeguro.pontos, executarAcao]);

  const removerPonto = useCallback((equipe: EquipeDaVez, quantidade = 1) => {
    const pontos = { ...estadoSeguro.pontos };
    if (equipe === "A") pontos.equipe1 = Math.max(0, pontos.equipe1 - quantidade);
    else pontos.equipe2 = Math.max(0, pontos.equipe2 - quantidade);
    executarAcao({ type: "REMOVER_PONTO", equipe, quantidade, pontos });
  }, [estadoSeguro.pontos, executarAcao]);

  const mostrarResposta = useCallback(() => executarAcao({ type: "MOSTRAR_RESPOSTA" }), [executarAcao]);
  const esconderResposta = useCallback(() => executarAcao({ type: "ESCONDER_RESPOSTA" }), [executarAcao]);
  ...
  const resetarPartida = useCallback(() => {
    ...
    inicial.pontos = { equipe1: 0, equipe2: 0 };
  ...
  const pergunta = partidaSegura.perguntasSelecionadas.find((item) => item.ordem === estadoSeguro.perguntaAtual)?.pergunta ?? null;
